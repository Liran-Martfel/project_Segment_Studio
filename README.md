...
- ✅ **Silhouette Score** — מחושב לצד WCSS עבור טווח ה-K שנבחר, מוצג יחד בטבלת שלב 2.
- ✅ **בחירת K אוטומטית** — כפתור בשלב 3: לפי Silhouette Score נבחר הערך הגבוה ביותר; אם אינו זמין, נשאל Ollama לגבי ה-K המומלץ לפי טבלת ה-WCSS.
- ✅ **ניקוי חריגים (Outliers)** — תיבת סימון בשלב 3 שמזהה נקודות עם מרחק חריג מהמרכז שלהן (מעל N סטיות תקן) ומסירה אותן מהקובץ המיוצא.
